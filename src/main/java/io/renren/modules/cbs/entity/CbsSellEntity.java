...
@Data
@TableName("cbs_sell")
public class CbsSellEntity extends BaseEntity implements Serializable {
    ...
    /**
     * 是否同步创建出库单 转化Boolean 需要在使用数据库加入&tinyInt1isBit=false
     */
    private Boolean synStoreOut;
    /**
    ...
    @TableField(exist = false)
    private List<CbsSellGoodsEntity> sellGoodsList;

    @ApiModelProperty("发票附件list")
    @TableField(exist = false)
    private List<CbsImgSellInvoiceEntity> imgSellInvoiceList;

    @ApiModelProperty("cbs_img_sell_delivery_order")
    @TableField(exist = false)
    private List<CbsImgSellDeliveryOrderEntity> imgSellDeliveryOrderList;

    @ApiModelProperty("cbs_img_sell_receipt")
    @TableField(exist = false)
    private List<CbsImgSellReceiptEntity> imgSellReceiptList;
